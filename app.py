...
ENCABEZADOS_USUARIOS = [
    ...
    "Teléfono", "Usuario", "Clave", "Tipo", "Fecha", "Push Token"
]

ENCABEZADOS_COLABORADORES = [
    "ID", "Nombre", "Primer apellido", "Segundo apellido",
    "Teléfono", "Usuario", "Clave", "Tipo", "Servicio", "Estado", "Fecha", "Push Token"
]
...
# =========================================================
# PARTE 2 / 5
# CONEXIÓN CON GOOGLE SHEETS, FIREBASE Y FUNCIONES AUXILIARES
# =========================================================
...
def crear_solicitud(servicio, usuario, detalle):
    ...
    notificar_colaboradores_disponibles(
        servicio,
        "🔔 Nueva solicitud",
        f"Hay una nueva solicitud de {servicio}: {limpiar_texto(detalle)}"
    )
...
# =========================================================
# ALERTAS SONORAS Y VISUALES INTERNAS
# =========================================================
...
def pagina_panel_admin():
    ...
    usuarios_con_push = [
        u for u in usuarios
        if u.get("Push Token", "").strip()
    ]

    colaboradores_con_push = [
        c for c in colaboradores
        if c.get("Push Token", "").strip()
    ]
    ...
    c7.metric("Usuarios con Push", len(usuarios_con_push))
    c8.metric("Colaboradores con Push", len(colaboradores_con_push))
    ...
    st.divider()
    st.subheader("🔔 Estado de notificaciones push")

    st.info(
        "Esta sección permite verificar quién ya tiene token guardado. "
        "Solo quienes tengan Push Token podrán recibir notificaciones reales."
    )

    col_push_1, col_push_2 = st.columns(2)

    with col_push_1:
        st.markdown("### 👤 Usuarios con Push")
        ...
        if usuarios:
            ...
            for u in usuarios:
                nombre = (
                    f'{u.get("Nombre", "")} '
                    f'{u.get("Primer apellido", "")} '
                    f'{u.get("Segundo apellido", "")}'
                ).strip()

                tabla_usuarios_push.append({
                    "Nombre": nombre,
                    "Usuario": u.get("Usuario", ""),
                    "Tiene Push": "Sí" if u.get("Push Token", "").strip() else "No"
                })

            st.dataframe(
                pd.DataFrame(tabla_usuarios_push),
                use_container_width=True
            )
        else:
    ...
    with col_push_2:
        st.markdown("### 🛠️ Colaboradores con Push")
        ...
        if colaboradores:
            ...
            for c in colaboradores:
                ...
                tabla_col_push.append({
                    "Nombre": nombre,
                    "Usuario": c.get("Usuario", ""),
                    "Servicio": c.get("Servicio", ""),
                    "Estado": c.get("Estado", ""),
                    "Tiene Push": "Sí" if c.get("Push Token", "").strip() else "No"
                })

            st.dataframe(
                pd.DataFrame(tabla_col_push),
                use_container_width=True
            )
        else:
    ...
    st.divider()
    st.subheader("🧪 Enviar prueba push")

    st.warning(
        "Use esta prueba solo para verificar que los tokens funcionan. "
        "El mensaje se envía al token seleccionado."
    )
    ...
    if tipo_prueba == "Usuario":
        ...
        if usuarios_push:
            opciones = {
                f'{u.get("Nombre", "")} {u.get("Primer apellido", "")} | {u.get("Usuario", "")}': u
                for u in usuarios_push
            }
            ...
            if st.button("Enviar prueba a usuario", use_container_width=True):
                u = opciones[seleccionado]

                ok = enviar_push_token(
                    u.get("Push Token", ""),
                    "🔔 Prueba Express Local",
                    "Esta es una prueba de notificación push para usuario."
                )

                if ok:
                    st.success("Notificación enviada correctamente.")
                else:
                    st.error("No se pudo enviar la notificación.")
        else:
            st.info("No hay usuarios con Push Token guardado.")
        ...
        if colaboradores_push:
            opciones = {
                f'{c.get("Nombre", "")} {c.get("Primer apellido", "")} | {c.get("Servicio", "")} | {c.get("Usuario", "")}': c
                for c in colaboradores_push
            }
            ...
            if st.button("Enviar prueba a colaborador", use_container_width=True):
                c = opciones[seleccionado]

                ok = enviar_push_token(
                    c.get("Push Token", ""),
                    "🔔 Prueba Express Local",
                    "Esta es una prueba de notificación push para colaborador."
                )
            ...
            st.info("No hay colaboradores con Push Token guardado.")
    ...
    if usuarios:
        for u in usuarios:
            ...
            with st.expander(f"👤 {nombre} | Usuario: {u['Usuario']}"):
                ...
                st.write(f"**Push activo:** {'Sí' if u.get('Push Token', '').strip() else 'No'}")
    ...
    if colaboradores:
        for c in colaboradores:
            ...
            with st.expander(
                ...
                st.write(f"**Push activo:** {'Sí' if c.get('Push Token', '').strip() else 'No'}")
